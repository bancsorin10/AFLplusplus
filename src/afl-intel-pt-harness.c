...
static void *load_bitmap(int *bitmap_fd) {
    ...
    char *bitmap_path = getenv("__AFL_SHM_ID");

    fd = shm_open(bitmap_path, O_RDWR | O_EXCL, 0600);
    if (fd <= 0) {
...
static __attribute__((constructor)) void main(int ac, char **av) {
    ...
    for (;;) {
        ...
        // reset bitmap
        // memset(bitmap, 0x00, MAP_SIZE);
        perf_fd = open_perf_event(pid);
        ...
        if (write(FORKSRV_FD + 1, &pid, 4) != 4) { exit(0); }

        if (write(FORKSRV_FD + 1, &main_ret, 4) != 4) { exit(0); }
